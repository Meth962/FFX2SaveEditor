    ...
    public class Ps3SaveManager
    {
        ...
        public Ps3SaveManager(string savedir, byte[] securefileid)
        {
            ...
            if (!File.Exists(savedir + "\\PARAM.PFD"))
                throw new Exception("Root directory does not contain any PARAM.PFD, Please load a valid directory");
            if (!File.Exists(savedir + "\\PARAM.SFO"))
                throw new Exception("Root directory does not contain any PARAM.SFO, Please load a valid directory");
            ...
            Files = (from ent in Param_PFD.Entries
                     let x = new FileInfo(savedir + "\\" + ent.FileName)
                     where x.Extension.ToUpper() != ".PFD" && x.Extension.ToUpper() != ".SFO"
                     select new Ps3File(savedir + "\\" + ent.FileName, ent, this)).ToArray();
        }
