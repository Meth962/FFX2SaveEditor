...
namespace PS3FileSystem
{
    public class Param_PFD
    {
        ...
        internal void Init(Stream input)
        {
            ...
            using (var br = new BinaryReader(input))
            {
                ...
                for (ulong i = 0; i < PFDHashTable.num_used; i++)
                {
                    ...
                    x.FileName = Encoding.ASCII.GetString(br.ReadBytes(65)).Replace("\0", "");
        ...
        internal byte[] GetEntryHash(int entryindex)
        {
            ...
            ulong tableindex = CalculateHashTableEntryIndex(ent.FileName);
            ...
            internal byte[] EntryData
            {
                get
                {
                    ...
                    using (var bw = new BinaryWriter(ms))
                    {
                        ...
                        Array.Copy(Encoding.ASCII.GetBytes(FileName), 0, name, 0, FileName.Length);
        ...
        public bool ValidFileCID(bool fix)
        {
            ...
            foreach (PFDEntry ent in PFDEntries.entries)
                indexes.Add((int)CalculateHashTableEntryIndex(ent.FileName));
        ...
        public bool ValidDHKCID2(bool fix)
        {
            for (int i = 0; i < PFDEntries.entries.Count; i++)
            {
                ...
                var index = (int)CalculateHashTableEntryIndex(PFDEntries.entries[i].FileName);
        ...
        {
            if (!input.CanRead)// Web only, we don't care about write || !input.CanWrite)
                throw new Exception("Unable to Access stream");
            input.Position = 0;
            foreach (PFDEntry t in PFDEntries.entries)
            {
                if (entryname.ToLower() == t.FileName.ToLower())
                {
                    for (int i = 0; i < 4; i++)
                    {
                        if (t.FileName.ToLower() == "param.sfo" && i != 0)
                            continue;
                        if (!pfd_istropy && i > 0)
                            continue;
        ...
        public bool ValidAllEntryHashes(string rootdirectory, bool fix)
        {
            for (int i = 0; i < PFDEntries.entries.Count; i++)
            {
                string filepath = rootdirectory + "\\" + PFDEntries.entries[i].FileName;
                if (!File.Exists(filepath))
                    return false;
        ...
        public bool EntryExists(string name)
        {
            if (PFDEntries.entries == null || PFDEntries.entries.Count == 0)
                return false;
            foreach (PFDEntry ent in PFDEntries.entries)
                if (ent.FileName.ToLower() == name.ToLower())
                    return true;
            return false;
        }
        ...
        public byte[] Decrypt(Stream stream, string entryname)
        {
            ...
            bool found = false;
            foreach (PFDEntry t in PFDEntries.entries)
                if (t.FileName.ToLower() == entryname.ToLower())
                {
                    ent = t;
                    found = true;
                    break;
        ...
        public int DecryptAllFiles(string root)
        {
            try
            {
                ...
                foreach (PFDEntry t in PFDEntries.entries)
                {
                    if (t.FileName.ToLower() == "param.sfo")
                        continue;
                    string filepath = root + "\\" + t.FileName;
                    if (File.Exists(filepath))
        ...
        public byte[] EncryptToBytes(Stream stream, string entryname)
        {
            if (SecureFileID == null || SecureFileID.Length != 16)
            {
                DoProgress(
        ...
        public bool Encrypt(byte[] inputdata, Ps3File entry)
        {
            try
            {
                ...
                byte[] data = EncryptToBytes(new MemoryStream(inputdata), entry.PFDEntry.FileName);
                if (data == null)
                    return false;
        ...
        public bool Encrypt(string filepath)
        {
            try
            {
                ...
                using (FileStream fs = File.Open(filepath, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
                {
                    for (int i = 0; i < PFDEntries.entries.Count; i++)
                    {
                        if (PFDEntries.entries[i].FileName.ToLower() == name.ToLower())
                        {
        ...
        public bool Encrypt(string filepath, string newfilepath)
        {
            try
            {
                ...
                FileStream fs = File.Open(filepath, FileMode.Open, FileAccess.ReadWrite, FileShare.None);
                byte[] data = EncryptToBytes(fs, name);
                fs.Close();
                fs.Dispose();
                DoProgress("Encrypting " + name + "..", MessageType.Info);
